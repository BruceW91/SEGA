...
class LLaVATrainer_DPO(Trainer):
    ...
    def concatenated_forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]],new_label) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        policy_chosen_logps , outputs = self.concatenated_forward(model, chosen_inputs, inputs['new_labels_chosen'])

        if has_true:

            policy_rejected_logps ,_ = self.concatenated_forward(model, rejected_inputs, inputs['new_labels_rejected'])
            new_chosen_inputs = {}
            ...
            with torch.no_grad():
                ...
                # reference_chosen_logps, reference_rejected_logps, _ = self.zb_concatenated_forward(self.reference_model, new_chosen_inputs, inputs['new_labels_chosen'][batch_flags], inputs['new_labels_rejected'][batch_flags]) 
                reference_chosen_logps , _ = self.concatenated_forward(self.reference_model, new_chosen_inputs, inputs['new_labels_chosen'])
                reference_rejected_logps , _ = self.concatenated_forward(self.reference_model, rejected_inputs, inputs['new_labels_rejected'])
            ...
            losses, chosen_rewards, rejected_rewards = self.preference_loss(
                    policy_chosen_logps[batch_flags], policy_rejected_logps, reference_chosen_logps, reference_rejected_logps, **loss_kwargs)
            losses = losses.mean()
