...
class LLaVATrainer_simPO(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        policy_chosen_logps, outputs = self.concatenated_forward(model, chosen_inputs, flag=1)

        if has_true:
            policy_rejected_logps, _ = self.concatenated_forward(model, rejected_inputs)
            ...
            losses, chosen_rewards, rejected_rewards = self.preference_loss(
                    policy_chosen_logps[batch_flags], policy_rejected_logps, **loss_kwargs)
            losses = losses.mean()
        else:
