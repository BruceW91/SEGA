...
# from ..utils import  load_remote_json
...
class LLaVATrainer(Trainer):
    def __init__(self, model, tokenizer, args, local_rank, data_module):
        ...
        try:
            self.jsons_dicts = pkl.load(open(data_path, "rb"))
            print('Validation dataset loaded from local!')
        except:
            pass
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            # 我感觉上是走这边 其实应该是只存lora部分的优化器状态就行吧
            ...
            optimizer_state_dict_path = os.path.join(output_dir, 'optimizer_state.pt')
