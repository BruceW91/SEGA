...
def get_bool_index(arrays):
    sorted_indices = torch.argsort(arrays, descending=True)  
    
    # 计算前50%的索引位置    改成12.5%了
    top_50_percent_index = len(arrays) // 4
    
    # 生成一个与原始tensor相同大小的bool数组，初始化为False  
    bool_array = torch.zeros(arrays.size(), dtype=torch.bool)  
    
    # 将前50%大的数据的对应位置设置为True  
    bool_array[sorted_indices[:top_50_percent_index]] = True  
...
class LLaVATrainer_DPO(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        with torch.no_grad():
            ...
            reference_chosen_logps, reference_rejected_logps, _ , ref_flag = self.zb_concatenated_forward(self.reference_model, copy.deepcopy(chosen_inputs), inputs['new_labels_chosen'], inputs['new_labels_rejected'],indexes)  # 这里为了兼容 multitask 跑全batch的推理了
        
        loss_kwargs = {'beta': 0.2, 'reference_free': False, 'label_smoothing': 0, 'ipo': False}
        
        if ref_flag < 0 or flag < 0:
