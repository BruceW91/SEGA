...
def evaluate(
        ...
        from ...dataset.subtask import crello_ocr as  ocr, crello_pred_continue as  pred_continue, crello_refine as  refine
        from ...dataset.subtask import crello_pred_simple  as pred
        from ...dataset.subtask import crello_pred_box as pred_box, crello_pred_raw as pred_raw
        ...
        for x in valset:
            ...
            try:
                flag = not flag
                dialog, image = pred_box.convert(x, flag)
                ...
                print(output)
                res = visualize_output_bb(dialog[1]['value'], image, (0,0,255))
                try:
                    res = visualize_output_bb(output, Image.fromarray(res), (255,0,0))
                except:
                    traceback.print_exc()
        ...
        if len(images_) > 0:
            ...
            trainer.tb_writer.add_images("BOX",np.array(images/255),global_step = trainer.state.global_step)
        ...
        for x in valset:
            ...
            try:
                flag = not flag
                dialog, image = pred_raw.convert(x, flag)
                output_ids = generate_bbox(image, dialog[0]['value'], 'poster_llava_general_wjh_0407', trainer.model, trainer.model.get_vision_tower().image_processor, trainer.tokenizer)
                output = trainer.tokenizer.batch_decode(output_ids)[0]
                print(output)
                res = visualize_output_bb(dialog[1]['value'], image, (0,0,255))
        ...
        if len(images_) > 0:
            images = combine_images(images_)[None].transpose(0,3,1,2)
            trainer.tb_writer.add_images("RAW",np.array(images/255),global_step = trainer.state.global_step)
            trainer.tb_writer.flush()
