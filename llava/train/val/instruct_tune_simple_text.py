...
def draw_one_wo_reshape(npimg, tp, colort):

    label = tp[0][:-1]
    output = ot = process_output_wo_reshape(tp[1], npimg.shape[:2][::-1])
    charnum = tp[2]
    fontsize = tp[3]
    color = tp[4]
    al = tp[5]
...
def visualize_output_partition_underlay(output, image, color):
    res = extract_bb(output)
    # print(res)
    npimg = np.array(rgba2rgb(image))
    ...
    for x in res:
        ...
        cur_category = x[0].strip('"')
        if cur_category == 'underlay':
            color = (0, 255, 0)
            # continue
        draw_one(npimg, x, color)
    return npimg

def visualize_output_bb_woreshape(output, image, color):
    res = extract_bb(output)
    print(res)
    npimg = np.array(rgba2rgb(image))
    for x in res:
        draw_one_wo_reshape(npimg, x, color)
    return npimg
...
def process_output(output,size):
    ...
    return output
def process_output_wo_reshape(output,size):
    # 把output x1,y1,x2,y2 转换为绝对坐标
    width, height = size

    out = [round((output[0]*width)), round((output[1]*height)), round((output[2]*width)), round((output[3]*height)) ]
    return out
...
def evaluate(
        ...
        from ...dataset.subtask import ocr, pred_continue, refine
        from ...dataset.subtask import pred_simple as pred
        from ...dataset.subtask import pred_text
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = pred_text.convert(x, flag)
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = pred.convert(x, flag)
                output_ids = generate_bbox(image, dialog[0]['value'], 'poster_llava_general_wjh_0407', trainer.model, trainer.model.get_vision_tower().image_processor, trainer.tokenizer)
                output = trainer.tokenizer.batch_decode(output_ids)[0]
                print(output)
                res = visualize_output(dialog[1]['value'], image, (0,0,255))
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = pred_continue.convert(x, flag)
                output_ids = generate_bbox(image, dialog[0]['value'], 'poster_llava_general_wjh_0407', trainer.model, trainer.model.get_vision_tower().image_processor, trainer.tokenizer)
                output = trainer.tokenizer.batch_decode(output_ids)[0]
                print(output)
                res = visualize_output(dialog[1]['value'], image, (0,0,255))
        ...
        if len(images_) > 0:
            images = combine_images(images_)[None].transpose(0,3,1,2)
            trainer.tb_writer.add_images("PRED_CONTINUE",np.array(images/255),global_step = trainer.state.global_step)
            trainer.tb_writer.flush()
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = refine.convert(x, flag)
                output_ids = generate_bbox(image, dialog[0]['value'], 'poster_llava_general_wjh_0407', trainer.model, trainer.model.get_vision_tower().image_processor, trainer.tokenizer)
                output = trainer.tokenizer.batch_decode(output_ids)[0]
                print(output)
                res = visualize_output(dialog[1]['value'], image, (0,0,255))
        ...
        if len(images_) > 0:
            images = combine_images(images_)[None].transpose(0,3,1,2)
            trainer.tb_writer.add_images("REFINE",np.array(images/255),global_step = trainer.state.global_step)
            trainer.tb_writer.flush()