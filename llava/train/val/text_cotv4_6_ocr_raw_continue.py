...
def evaluate(
        ...
        from ...dataset.subtask import pred_textnoc_cotv4_6, ocr_text, pred_raw_nocate, text_continue_cotv4_6
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = pred_textnoc_cotv4_6.convert(x, flag)
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = pred_raw_nocate.convert(x, flag)
                output_ids = generate_bbox(image, dialog[0]['value'], 'poster_llava_general_wjh_0407', trainer.model, trainer.model.get_vision_tower().image_processor, trainer.tokenizer)
                output = trainer.tokenizer.batch_decode(output_ids)[0]
                print(output)
                res = visualize_output(dialog[1]['value'], image, (0,0,255))
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = ocr_text.convert(x, flag)
                output_ids = generate_bbox(image, dialog[0]['value'], 'poster_llava_general_wjh_0407', trainer.model, trainer.model.get_vision_tower().image_processor, trainer.tokenizer)
                output = trainer.tokenizer.batch_decode(output_ids)[0]
                print(output)
                res = visualize_output(dialog[1]['value'], image, (0,0,255))
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = text_continue_cotv4_6.convert(x, flag)
                output_ids = generate_bbox(image, dialog[0]['value'], 'poster_llava_general_wjh_0407', trainer.model, trainer.model.get_vision_tower().image_processor, trainer.tokenizer)
                output = trainer.tokenizer.batch_decode(output_ids)[0]
                print(output)
                res = visualize_output(dialog[1]['value'], image, (0,0,255))
        ...
        if len(images_) > 0:
            images = combine_images(images_)[None].transpose(0,3,1,2)
            trainer.tb_writer.add_images("CONTINUE",np.array(images/255),global_step = trainer.state.global_step)
            trainer.tb_writer.flush()
