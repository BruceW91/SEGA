...
def change_lay(inp, dst):
    a = inp.split('''[{"category"''')

    b = a[0].split('input: ')

    b[-1] = f'The global layout should be {dst}. '
    a[0] = 'input: '.join(b)
    return '''[{"category"'''.join(a)
...
def evaluate(
        ...
        from ...dataset.subtask import pred_desired_cotv2
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = pred_desired_cotv2.convert(x, flag)
        ...
        if len(images_) > 0:
            ...
            trainer.tb_writer.add_images("PRED",np.array(images/255),global_step = trainer.state.global_step)
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = pred_desired_cotv2.convert(x, flag)
                output_ids = generate_bbox(image, change_lay(dialog[0]['value'], 'horizontal-bottom'), 'poster_llava_general_wjh_0407', trainer.model, trainer.model.get_vision_tower().image_processor, trainer.tokenizer)
                output = trainer.tokenizer.batch_decode(output_ids)[0]
                print(output)
                res = visualize_output(dialog[1]['value'], image, (0,0,255))
                try:
        ...
        if len(images_) > 0:
            images = combine_images(images_)[None].transpose(0,3,1,2)
            trainer.tb_writer.add_images("horizontal-bottom",np.array(images/255),global_step = trainer.state.global_step)
            trainer.tb_writer.flush()
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = pred_desired_cotv2.convert(x, flag)
                output_ids = generate_bbox(image, change_lay(dialog[0]['value'], 'vertical-left'), 'poster_llava_general_wjh_0407', trainer.model, trainer.model.get_vision_tower().image_processor, trainer.tokenizer)
                output = trainer.tokenizer.batch_decode(output_ids)[0]
                print(output)
                res = visualize_output(dialog[1]['value'], image, (0,0,255))
                try:
        ...
        if len(images_) > 0:
            images = combine_images(images_)[None].transpose(0,3,1,2)
            trainer.tb_writer.add_images("vertical-left",np.array(images/255),global_step = trainer.state.global_step)
            trainer.tb_writer.flush()
        ...
        for x in valset:
            try:
                flag = not flag
                dialog, image = pred_desired_cotv2.convert(x, flag)
                output_ids = generate_bbox(image, change_lay(dialog[0]['value'], 'top-right'), 'poster_llava_general_wjh_0407', trainer.model, trainer.model.get_vision_tower().image_processor, trainer.tokenizer)
                output = trainer.tokenizer.batch_decode(output_ids)[0]
                print(output)
                res = visualize_output(dialog[1]['value'], image, (0,0,255))
                try:
        ...
        if len(images_) > 0:
            images = combine_images(images_)[None].transpose(0,3,1,2)
            trainer.tb_writer.add_images("top-right",np.array(images/255),global_step = trainer.state.global_step)
            trainer.tb_writer.flush()
