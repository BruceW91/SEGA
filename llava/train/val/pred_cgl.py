...
def remove_chars_before_first_digit(s: str) -> str:
    ...
    # If there's no digit, return the original strin[g
    if index is None:
        return s
...
def evaluate(
        ...
        from ...dataset.subtask import pred_cgl
        #valset = trainer.train_dataset.list_data_dict
        #valset = valset[::len(valset)//5][:4]
        # with open('/root/paddlejob/workspace/log/code/qiutingxuan/baidu/personal-code/TypoLMM/LLaVA-main/cgl_val.pkl','rb') as f:
        #     valset = pkl.load(f)
        valset = trainer.jsons_dicts
        valset = valset[:10]
        flag = True
        images_ = []
        for x in valset:
            try:
                flag = not flag
                dialog, image = pred_cgl.convert(x, flag)
                ...
                gt_text = dialog[1]['value']
                gt_list = eval(gt_text)

                bg = np.array(image)
                for ix, (output, gt) in enumerate(zip(output_list,gt_list)):
                    pred_bbox = process_output(str(output['bbox']).replace('[','').replace(']',''),image.size)
                    gt_bbox = process_output(str(gt['bbox']).replace('[','').replace(']',''),image.size)
                    print(pred_bbox,gt_bbox)
                    
                    cv2.rectangle(bg,tuple(pred_bbox[:2]),tuple(pred_bbox[2:]),(255,0,0),3)
                    cv2.rectangle(bg,tuple(gt_bbox[:2]),tuple(gt_bbox[2:]),(0,0,255),1)
                    cv2.putText(img=bg, text = str(ix)+output['category'], org=tuple(pred_bbox[:2]),fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.4,color=(255,0,0),thickness=3)
                    cv2.putText(img=bg, text = str(ix)+gt['category'], org=tuple(gt_bbox[:2]),fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.4,color=(0,0,255),thickness=1)

  
                images_.append(process_image(bg))
            except:
                traceback.print_exc()
        if len(images_) > 0:
            images = combine_images(images_)[None].transpose(0,3,1,2)
            trainer.tb_writer.add_images("PRED"+str(trainer.state.global_step),np.array(images/255),global_step = trainer.state.global_step)
