...
def get_cot_prompt_for_align(scores):
    total_prompt = []
    thresholds = [0.001, 0.24, 0.12, 0.38, 0.03, 0.008, 0.95, 0.95]
    if scores[0] > thresholds[0]:
        # total_prompt.append("Text elements require stricter alignment")
        pass
    if scores[1] > thresholds[1]:
...
def convert(s, flip, scores, temp_pred):
    ...
    need_list_for_draw_bbox = [ [item['label'], item['Bounding Box'] ] for item in pred_layers if item['label'] != 'underlay' ] 
    # 之前这边的图 没画进去
    # 这里改成自己的新画框函数 ！！！
    # inpimg = zb_draw_all_crello(pred_layers,(bgimg.shape[1], bgimg.shape[0]), bgimg, layers, flip).convert('RGB')
    inpimg = visualize_output(need_list_for_draw_bbox, bgimg)
   ...
   'value': '''<image>\n Given a poster image ,a series of text to be added to the poster subsequently and a poster laypout, please refine the layouts and give reasons.\n''' + \
        f'''input: {iper}Layouts to be refined:" {need_str}'''},
        {
            'from': 'gpt',
...
def convert_no_score(s, flip, temp_pred):
