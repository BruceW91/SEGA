...
def convert(pk, flip):
    ...
    try:
        ...
        print(e)
        for x in layers:
            print(str(x['org']), str(x['Bounding Box']), str(bgimg.size))
        raise e
    ...
    [(x.pop('img'), x.pop("Bounding Box")) for x in orglayers]
    iper = add_newline(json.dumps(layers))
    dialog = [
        {'from': 'human',
   'value': '''<image>\n Given a poster image drawn with a series of text, first detect all text blocks, then modify the problematic metadata within it to make the refined visuals reasonable and aesthetically pleasing. The answer should be written in json format. Each json should have a key “modified” indicating whether the original json need to be modified.'''},
        {
            'from': 'gpt',
