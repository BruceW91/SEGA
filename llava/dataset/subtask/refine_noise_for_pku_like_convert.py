...
# 基于这个没有噪声的版本写
def convert_no_score(s, flip, temp_saliency_map, bgimg):
    ...
    cur_pred ,prompt_list = get_cot_prompt_new(copy.deepcopy(layers), udl, temp_saliency_map, img_)

    new_absolute_bboxes = copy.deepcopy([x['bbox'] for x in cur_pred ]) 
    for item in cur_pred:
        r_bbox = json.loads(get_bbox_tokens_2(item['bbox'], (w,h)))
        item['bbox'] = r_bbox

    pred_layers_for_bbox = [  ]
    for element in cur_pred:
        # if 'Bounding Box' not in list(element.keys()):
        #     print(element)
            # element['Bounding Box'] = element['bbox']
        cur = [element['category'], element['Bounding Box']]
        pred_layers_for_bbox.append(cur)
    
    inpimg = visualize_output(pred_layers_for_bbox, np.array(bgimg))
    # inpimg = zb_draw_all_crello_for_readnoise(new_absolute_bboxes, (w, h), img_, copy.deepcopy(reduncy_layers), flip).convert('RGB')  # 全画上去
