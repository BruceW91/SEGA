...
def convert(s, flip, ratio_0=False):
    ...
    # 注意这里 改成不reshape的bbox了

    layers = convert_dct_list_crello_get_bbox_tokens_2_wo_reshapebbox(s['layers'])
...
def convert_and_merge_2_dialog(s, flip, example_s,cot_flag = False):
   ...
   'value': '''Given a half-finished poster image and a series of text to be added to the poster subsequently, predict the metadata for each text metadata listed below. Firstly, learn an example and then provide an answer\n''' + \
        ...
        {
            'from': 'gpt',
            'value': f'previous: {dd}\nunderlay: {judl}\n'+add_newline(json.dumps(e_layers[0:]))
        }
    ]    
    return dialog, img, e_img

def convert_and_merge_2_dialog_for_look(s, flip, example_s,cot_flag = False):
    ...
    text_gt_cur = f'previous: {dd}\nunderlay: {judl}\n'+add_newline(json.dumps(layers))
    text_gt_e = f'previous: {e_dd}\nunderlay: {e_judl}\n'+add_newline(json.dumps(e_layers))
    ...
    return text_gt_cur, text_gt_e, img, e_img