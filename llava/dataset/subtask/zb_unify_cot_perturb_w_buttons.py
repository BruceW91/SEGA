...
def zb_perturb_bbox(bbox, translate_ratio=0.2, scale_ratio=0.1):
    """
    ...
    return [round(val, 3) for val in [new_x_min, new_y_min, new_x_max, new_y_max]], scale_factor
...
def convert(pk, flip, in_layer=-1):
    ...
    for x in layers:
        ...
        if True:  # 永久扰动
            # y['modified'] = True
            # temp_bbox = normalize_bbox(x['Bounding Box'], bgimg.size)
            # y['Bounding Box'], ratio = perturb_bbox(x['Bounding Box'], bgimg.size)
            temp_bbox,t_ratio = zb_perturb_bbox(x['bbox'])
            y["fontsize"] = np.around(x["fontsize"] * t_ratio, 2)
            y['bbox'] = temp_bbox
