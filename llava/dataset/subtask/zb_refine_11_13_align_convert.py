...
def convert(s, flip, scores, temp_pred):
    ...
    sorted_need_list = sorted(need_list, key=lambda x: x['category'])
    need_str = json.dumps(sorted_need_list)

    for ubbox in udl_bbox:
        cur_item = {'category':'underlay','bbox':ubbox}
        layers.append(cur_item)
    sorted_layers = sorted(layers, key=lambda x: x['category'])

    dialog = [
        {'from': 'human',
   'value': '''<image>\n Given a poster image and a poster laypout, please evaluate and refine the layouts.\n''' + \
        f'''input: Layouts to be refined:" {need_str}'''},
        {
            'from': 'gpt',
            'value': f'{reasons}\n'+add_newline(json.dumps(sorted_layers))
        }
    ]
...
def convert_no_score(s, flip, temp_pred):
    ...
    # prompt_list = get_cot_prompt(scores)
    # total_str = ""
    # for i in range(len(prompt_list)):
    #     total_str += f"{i}. {prompt_list[i]} "
