...
from llava.dataset.zb_refine_noise_utils.add_noise_for_pku import total_add_noise_pku
import copy
...
def get_cot_prompt_no_detail(layers, temp_saliency_map):
    total_prompt = []
    origin_layers = layers.copy()

    #total_加噪声  最后给出三种类型吧 要不就弄三个列表 然后根据这三个列表生产prompt
    total_layouts, occlusion_errors, err_underlay, overlay_errors = total_add_noise_pku(layers, temp_saliency_map)
...
def get_cot_prompt_for_new(layers, temp_saliency_map, cur_img):
    ...
    scores_line = {'overlay':0.0008,'utilization':0.16,'underlay_effectiveness_strict':0.9,'occlusion':0.20}
    # 针对cgl 来一个
