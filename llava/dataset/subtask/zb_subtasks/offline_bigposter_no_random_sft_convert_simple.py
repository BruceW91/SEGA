...
def convert(pk, flip):
    ...
    texts = [ item['Text'] for item in layers]
    gua = []
    w, h = bgimg.size
    for but in pk[-1]:
    ...
    [(x.pop('thought'), x.pop('buttons'), x.pop('Text'), x.pop('img'), x.pop("Bounding Box"),x.pop("alignment"),x.pop("fontsize"),x.pop('fontcolor')) for x in layers]
   ...
   'value': '''<image>\n Given a half-finished poster image and a series of text to be added to the poster subsequently, predict the metadata for each text metadata listed below.''' + \
    ...
    return dialog, inpimg, texts

from copy import deepcopy
def convert_for_infer(pk, flip):
    bg = pk[0]
    # bgimg = download_image(bg)
    bgimg = get_my_poster_png(bg)
    ratio = 0
    layers, _ = convert_dct_list_text_buttons(pk)
    origin_layers = deepcopy(layers)
    texts = [ item['Text'] for item in layers]
    gua = []
    w, h = bgimg.size
    for but in pk[-1]:
   ...
   'value': '''<image>\n Given a half-finished poster image and a series of text to be added to the poster subsequently, predict the metadata for each text metadata listed below.''' + \
    ...
    return dialog, inpimg, texts, origin_layers