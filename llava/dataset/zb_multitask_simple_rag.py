...
# import 子任务
from .subtask import crello_ocrv2_drop, crello_pred_simple, zb_simple_crello_rag_convert, zb_crello_cot_simple_for_multi_task

def select_task(pk,example_pk):
    '''
    这里我采用两个转换任务 按比例选取的方式
    '''

    s = pkl.load(open(pk, 'rb'))
    e_s = pkl.load(open(example_pk, 'rb'))
    flip = np.random.rand() > 0.5
    return  np.random.choice( [zb_simple_crello_rag_convert.convert_and_merge_2_dialog, zb_crello_cot_simple_for_multi_task.convert])(s, flip, e_s) 
    ...
    def __getitem__(self, i):
        while True:
            try:
                ...
                temp_rag_index = random.choice(self.rag_indexes[temp_name])
