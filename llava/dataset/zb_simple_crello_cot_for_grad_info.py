...
from llava.dataset.subtask.zb_crello_cot_simple import convert_dct_list_crello_simple
# import 子任务
from llava.dataset.subtask import crello_pred_continuev2_underlay_tuili, crello_pred_raw_continuev2_underlay_tuili,zb_crello_cot_simple

def select_task(pk):
    s = pkl.load(open(pk, 'rb'))
    flip = np.random.rand() > 0.5
    return np.random.choice((zb_crello_cot_simple.convert, ))(s, flip, True)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        rank0_print("real_len:",len(list_data_dict))
    ...
    def __getitem__(self, i):
        while True:
            try:
                ...
                name = sources.name
                dialog, image, text_layers_len = select_task(sources)
...
#     ddd = LazySupervisedDataset('new_crello_01', None, None)
#     print("jj")