...
# import 子任务
from .subtask import zb_origin_no_pred_continue_cot,zb_no_pred_continue_cot_perturb,zb_unify_cot_perturb,zb_unify_cot_perturb_w_buttons
...
def select_task(pk):
    ...
    return zb_unify_cot_perturb_w_buttons.convert(pk, flip)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        v1_temp_p = 'infer_out_for_train_DPO/dpo_look_random_1.5pkl'
        self.ps = list(Path(v1_temp_p).glob('*.pkl'))
        try:
            with open(data_path, 'rb') as file:
                list_data_dict = pkl.load(file)  
            # with open(v1_temp_p, 'rb') as file:
            #     self.v1_results = pkl.load(file)  
    ...
    def __getitem__(self, i):
        while True:
            try:
                ...
                if i < 50:
                    worst_pred_P = self.ps[i]
                    final_path = os.path.join(str(worst_pred_P.parent) ,str(i)+".pkl")  
                    s = pkl.load(open(final_path, 'rb'))
                    print("hhh")
