...
# import 子任务
from .subtask import crello_ocr, crello_pred_simple, crello_pred_continue, crello_refine, crello_pred_box, crello_pred_raw
...
def select_task(pk):
    ...
    sd = np.random.rand()
    if sd > 0.4:
        return crello_pred_simple.convert(s, flip)
    else:
        return np.random.choice((crello_ocr.convert, crello_pred_raw.convert))(s, flip)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        list_data_dict = list(Path(data_path).glob('*.pkl'))
