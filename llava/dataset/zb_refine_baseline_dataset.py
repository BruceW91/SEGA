...
# import 子任务
from .subtask import zb_simple_crello_rag_convert, zb_simple_rag_convert_fix_underlay, zb_refine_convert_baseline
# 注意这里更换了 fix underlay prompt的convert函数

# 下面应该是 

def select_task(pk,scores,temp_pred):

    s = pkl.load(open(pk, 'rb'))
    flip = np.random.rand() > 0.5
    return zb_refine_convert_baseline.convert(s, flip, scores,temp_pred)
    ...
    def __getitem__(self, i):
        while True:
            try:
                ...
                dialog, image = select_task(sources,scores,temp_pred)
