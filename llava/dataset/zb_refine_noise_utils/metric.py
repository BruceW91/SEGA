...
def _mean_for_underlay_zb(values: List[float]) -> Optional[float]:
    if len(values) == 0:
        return 1.0
    else:
        return sum(values) / len(values)
...
def __compute_overlay_zb(layout: Layout) -> Optional[float]:
    """
    Average IoU except underlay components used in PosterLayout.
    Lower values are better (in 0.0 - 1.0 range).
    """
...
def compute_overlay_zb(
    ...
    results: dict[str, List[float]] = {
        "overlay": run_parallel(__compute_overlay_zb, layouts)
    }
    return results
...
def __compute_underlay_effectiveness_zb(
    layout: Layout,
    underlay_id: int,
) -> dict[str, Optional[float]]:
    """
    ...
    return {
        "underlay_effectiveness_loose": _mean_for_underlay_zb(loose_scores),
        "underlay_effectiveness_strict": _mean_for_underlay_zb(strict_scores),
    }
...
def compute_underlay_effectiveness_zb(
    batch: dict[str, Tensor],
    feature_label: ds.ClassLabel,
) -> dict[str, List[float]]:
    """
    ...
    results: dict[str, List[float]] = run_parallel(
        partial(__compute_underlay_effectiveness_zb, underlay_id=underlay_id), layouts
    )
    return results
...
def compute_saliency_aware_metrics(
    ...
    - unreadability:
        ...
        Lower values are generally better.
    """
    text_id = feature_label.str2int("Title")
    underlay_id = feature_label.str2int("Underlay")
...
class _TimmVGGWrapper(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        # )
        self.model = timm.create_model("vgg16.tv_in1k", checkpoint_path='cache/PRECOMPUTED_WEIGHT_DIR/fidnet/vgg/pytorch_model.bin', num_classes=0).to(
            torch.device(device_name)
        )
        self.model.eval()
...
class _TimmInceptionV3Wrapper(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.model = timm.create_model(
            #"inception_v3", checkpoint_path='/root/paddlejob/workspace/log/code/qiutingxuan/RALF/RALF-main/cache/PRECOMPUTED_WEIGHT_DIR/fidnet/inception_v3/model.safetensors', num_classes=0
            "inception_v3", checkpoint_path='cache/PRECOMPUTED_WEIGHT_DIR/fidnet/inception_v3/pytorch_model.bin', num_classes=0
        ).to(torch.device(device_name))
        self.model.eval()
        data_config = timm.data.resolve_model_data_config(self.model)
        # transform = timm.data.create_transform(**data_config, is_training=False)
...
def zb_compute_validity(
    ...
    for d in data:
        ...
        N_denominator += len(is_valid)
        N_numerator += is_valid.count(True)
        individual_scores.append(is_valid.count(True) / len(is_valid))
    ...
    validity = N_numerator / N_denominator
    return filtered_data, validity, individual_scores
