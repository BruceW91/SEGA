...
def get_scores_for_cur_layouts(layouts,saliency_map, cur_img):
    ...
    # 这里我需要准备好 saliency map   以及对layouts 转换为 需要的格式
    # 先转成字典 再collate
...
def add_noise_and_get_score_for_crello(layouts, underlays, saliency_map, cur_img):
    ...
    while(True):
        ...
        if cur_p < 0.6 :
            ...
            break
    if len(underlays)>0:
        underlay_layouts = [ {'category':'underlay', 'bbox':bbox} for bbox in underlays ]
        total_layouts_for_scores =  layouts + underlay_layouts
    else:
        total_layouts_for_scores =  layouts

    # 这边主要是需要layouts 绝对坐标来算分

    # 或许当前布局的先验分数, 下面再根据这个分数 卡阈值 来给出cot句子
    scores_list = get_scores_for_cur_layouts(total_layouts_for_scores, saliency_map, cur_img)
