...
def get_cot_prompt(layers, underlays, temp_saliency_map):
    ...
    if len(err_underlay)>0:
        total_prompt.append("There are some underlays without any text elements."+ err_underlay_str)
...
def convert_no_score(s, flip, temp_saliency_map):
    ...
    cur_pred ,prompt_list = get_cot_prompt(copy.deepcopy(layers), udl_bbox, temp_saliency_map)
