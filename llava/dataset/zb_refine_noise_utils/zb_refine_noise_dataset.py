...
# import 子任务
from .subtask import zb_simple_crello_rag_convert, zb_simple_rag_convert_fix_underlay, zb_refine_convert_baseline, zb_refine_noise_convert
# 注意这里更换了 fix underlay prompt的convert函数

# 这边我的计划是做一个纯噪声的版本看看效果
# 那我其实就要在洗过的数据上做


def select_task(pk,temp_saliency_map):

    s = pkl.load(open(pk, 'rb'))
    flip = np.random.rand() > 0.5
    return zb_refine_noise_convert.convert_no_score(s, flip, temp_saliency_map)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        #     try:
            ...
            dialog, image = select_task(sources,temp_saliency_map)
