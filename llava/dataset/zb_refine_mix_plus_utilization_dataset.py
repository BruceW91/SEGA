...
# import 子任务
from .subtask import  zb_refine_convert_baseline, zb_refine_noise_convert, zb_refine_baseline_plus_allign, zb_refine_no_detail_noise_plus_convert, zb_refine_no_detail_noise_plus_add_utilization_convert, zb_refine_baseline_plus_add_utilization_allign
...
def select_task(pk, score, temp_pred, temp_saliency_map):
    ...
    # 目前对齐和随机1:1
    if select_score < 0.5:
        return zb_refine_no_detail_noise_plus_add_utilization_convert.convert_no_score(s, flip, temp_saliency_map)
    else:
        return zb_refine_baseline_plus_add_utilization_allign.convert(s, flip, score, temp_pred)
