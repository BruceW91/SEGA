...
from llava.dataset.subtask import crello_sft_like_pku_convert

def select_task(pk,bgimg):
    s = pkl.load(open(pk, 'rb'))
    flip = np.random.rand() > 0.5
    return np.random.choice((crello_sft_like_pku_convert.convert, ))(s, flip, bgimg)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        self.img_folder = '/root/paddlejob/workspace/log/code/RALF-main/zb_utils_data/crello_inpainting/input'
    ...
    def __getitem__(self, i):
        while True:
            try:
                ...
                name = sources.name
                img_p = os.path.join(self.img_folder, name.replace('.pkl', '.png'))
                image = Image.open(img_p)
                dialog, text_layers_len = select_task(sources,image)
