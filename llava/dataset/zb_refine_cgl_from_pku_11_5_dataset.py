...
# import 子任务
from .subtask import zb_final_cgl_align_convert_from_pku, zb_final_cgl_noise_convert_from_pku
...
def select_task(content, score, temp_pred, temp_saliency_map, img_folder_p):
    ...
    # 目前对齐和随机1:1
    if select_score < 0.5:
        return zb_final_cgl_noise_convert_from_pku.convert(content, img_folder_p, temp_saliency_map)
    else:
        return zb_final_cgl_align_convert_from_pku.convert(content, score, temp_pred, img_folder_p)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        pku_train_p = '/root/paddlejob/workspace/log/code/others/cgl_dataset/real_train_annotation_xyxy.pkl'  # ok
        self.pku_inpainted_imgs_p = '/root/paddlejob/workspace/log/code/others/cgl_dataset/cgl_inpainting_all/' #ok
        pku_data = pkl.load(open(pku_train_p, 'rb'))
        imgs_names = os.listdir(self.pku_inpainted_imgs_p) 

        scores_p = 'zb_data/sft_nocot_3500_trainpkl.pkl' # ok
        ...
        self.pred_p = 'infer_out_cgl/sft_nocot_3500_trainpkl' # ok
        self.saliency_map_folder = 'llava/dataset/zb_refine_noise_utils/cgl_union_sal' # ok
